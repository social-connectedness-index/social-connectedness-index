...
0. **Web app (no install)** — Make maps right in your browser at
   **[social-connectedness.org](https://social-connectedness.org/)** — no R, no
   ...
   single-region and comparison maps with PNG/JPG/SVG/MP4 downloads. The site also
   includes the Interactive Explorer, Connected Communities clustering tool, and
   Cross-Gender Friending Ratio visualizer.
   Source and deploy instructions are in [`web/README.md`](web/README.md).
